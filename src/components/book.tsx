...
/* Icoon per podcastplatform. Spotify houdt zijn eigen logo; Podimo krijgt een
   neutrale koptelefoon in de kleuren van deze sectie. Geen nagetekend merklogo,
   en een externe logo-afbeelding blokkeert de CSP (img-src 'self'). */
function PodcastIcon({ platform }: { platform: string }) {
  if (platform === "spotify") {
    return (
  ...
  return (
    <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#7da7f0]/15 text-[#7da7f0]">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5">
        <path d="M4 15v-3a8 8 0 0 1 16 0v3" />
        <path d="M4 15a2 2 0 0 1 2-2h1a1 1 0 0 1 1 1v5a1 1 0 0 1-1 1H6a2 2 0 0 1-2-2v-3z" />
        <path d="M20 15a2 2 0 0 0-2-2h-1a1 1 0 0 0-1 1v5a1 1 0 0 0 1 1h1a2 2 0 0 0 2-2v-3z" />
      </svg>
    </div>
  );
}
              ...
              {book.podcasts.map((podcast) => (
                ...
                >
                  ...
                  <div>
                    ...
                    {podcast.note && <p className="mt-2 text-xs text-[#7da7f0]">{podcast.note}</p>}
                  </div>
